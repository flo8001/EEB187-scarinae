...
```{r}
library(pavo); library(ape); library(phytools); library(caper)
library(nlme); library(plotrix); library(tidyr); library(ggplot2); library(here); library(tidyverse)
stopifnot(requireNamespace("png", quietly = TRUE))         # biplot thumbnails
stopifnot(packageVersion("rfishbase") == "3.1.10")          # §2.8 FishBase pull
cat("YOU'RE READY FOR WEDNESDAY!\n")
```
http://164.67.78.200:5052/api/export-bundle/scarinae?smoke=true

```{r}
set.seed(2026)
bundle <-  here("data/scarinae-78-bundle")   # your own family: e.g. "acanthuridae-mini-bundle"

# Load pavo pipeline + bundle-loading helpers
source(here("scripts/measurement-error.R"))
```
...
plot_pca_images(pc$x, pc$rotation, var_pct, bundle,
                main = "scarinae pattern PCA · biplot (no tree)")
...
if (!exists("tree"))           tree           <- read.tree(file.path(bundle, "scarinae-mini-tree.tre"))
...
plot_pca_images(
  ...
  main         = "scarinae pattern phylo-PCA · biplot"
)
...
# ---- 2.6 Compare to Frédérich K_mult = 0.10 (scarinae) -------
...
```{r}
#write_csv(traits_df, "traits_df_78.csv")
```
